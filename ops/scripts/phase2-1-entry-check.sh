...
out="${FABRIC_REPO_ROOT}/acceptance/PHASE2_1_ENTRY_CHECKLIST.md"
...
{
  echo "# Phase 2.1 Entry Checklist"
  echo
  ...
  echo
  echo "2) REQUIRED-FIXES.md has no OPEN items"
  ...
  echo
  echo "3) No insecure TLS flags in automation paths"
  ...
  if rg -n "pm_tls_insecure|PM_TLS_INSECURE|curl -k|--insecure" -S \
    ...
    --glob '!phase2-1-entry-check.sh' >/dev/null 2>&1; then
    fail
  else
    echo "- Result: PASS"
  fi
  echo
  echo "4) Proxmox API token only (no root@pam, no node SSH/SCP)"
  ...
  echo
  echo "5) Inventory contract present"
  ...
  echo
  echo "6) Runner env presence (safe check)"
  ...
  echo
  echo "Notes:"
  echo "- If any criterion fails, Phase 2.1 work must stop and remediation must be recorded in REQUIRED-FIXES.md."
} > "${out}"
