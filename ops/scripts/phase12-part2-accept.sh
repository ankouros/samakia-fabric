...
marker="${acceptance_dir}/PHASE12_PART2_ACCEPTED.md"

run_step() {
  local label="$1"
  shift
  echo "[phase12.part2] ${label}"
  "$@"
}
...
run_step "Secrets materialize (dry-run)" make -C "${FABRIC_REPO_ROOT}" bindings.secrets.materialize.dryrun TENANT=all
run_step "Secrets rotate plan" make -C "${FABRIC_REPO_ROOT}" bindings.secrets.rotate.plan TENANT=all
run_step "Secrets rotate dry-run" make -C "${FABRIC_REPO_ROOT}" bindings.secrets.rotate.dryrun TENANT=all
run_step "Phase 12 Part 2 entry check" make -C "${FABRIC_REPO_ROOT}" phase12.part2.entry.check
