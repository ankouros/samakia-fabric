...
if [[ -z "${db_name}" ]]; then
  echo "ERROR: resources.database is required for mariadb backup" >&2
  exit 2
fi
...
mkdir -p "${out_dir}"
backup_file="${out_dir}/${db_name}.sql"

mysqldump -h "${host}" -P "${port}" -u "${admin_user}" -p"${admin_pass}" "${db_name}" >"${backup_file}"
...
echo "PASS mariadb backup: ${backup_file}"
