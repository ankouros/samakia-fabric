...
run_for_tenant() {
  ...
  contracts_json="$(list_enabled_contracts "${tenant_dir}" "postgres")"
  echo "${contracts_json}" | jq -c '.[]' | while read -r entry; do
    ...
    local secret_json admin_user admin_pass app_user app_pass db_name schema_name
...
PY
    ...
    schema_name=$(python3 - <<PY
import json
print(json.loads('''${resources}''').get('schema', '') or '')
PY
)
...
SQL
    ...
    if [[ -n "${schema_name}" ]]; then
      PGPASSWORD="${admin_pass}" psql -v ON_ERROR_STOP=1 -h "${host}" -p "${port}" -U "${admin_user}" -d "${db_name}" <<SQL
CREATE SCHEMA IF NOT EXISTS "${schema_name}" AUTHORIZATION "${app_user}";
SQL
    fi

    PGPASSWORD="${admin_pass}" psql -v ON_ERROR_STOP=1 -h "${host}" -p "${port}" -U "${admin_user}" -d "${db_name}" -c "SELECT 1;" >/dev/null
...
EOF_REPORT
  ...
  echo "PASS substrate apply (postgres): ${out_dir}"
}
