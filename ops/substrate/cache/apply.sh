...
run_for_tenant() {
  ...
  echo "${contracts_json}" | jq -c '.[]' | while read -r entry; do
    ...
    if [[ "${resources}" == "{}" ]]; then
      echo "ERROR: resources are empty for ${tenant_id}/${consumer}/${provider}; specify tenant_key_prefix and max_memory_mb" >&2
      exit 2
    fi

    local secret_json password key_prefix max_memory eviction_policy
    secret_json=$(load_secret_json "${secret_ref}")
    password=$(get_secret_field "${secret_json}" "password")

    key_prefix=$(python3 - <<PY
import json
print(json.loads('''${resources}''').get('tenant_key_prefix', '') or json.loads('''${resources}''').get('key_prefix', '') or '')
PY
)
    max_memory=$(python3 - <<PY
import json
print(json.loads('''${resources}''').get('max_memory_mb', '') or '')
PY
)
    eviction_policy=$(python3 - <<PY
import json
print(json.loads('''${resources}''').get('eviction_policy', '') or '')
PY
)

    if [[ -z "${key_prefix}" ]]; then
      echo "ERROR: resources.tenant_key_prefix is required for ${tenant_id}/${consumer}/${provider}" >&2
      exit 2
    fi

    if [[ -z "${max_memory}" ]]; then
      echo "ERROR: resources.max_memory_mb is required for ${tenant_id}/${consumer}/${provider}" >&2
      exit 2
    fi

    if [[ -z "${eviction_policy}" ]]; then
      echo "ERROR: resources.eviction_policy is required for ${tenant_id}/${consumer}/${provider}" >&2
      exit 2
    fi

    if [[ -z "${password}" ]]; then
      echo "ERROR: password missing in secret_ref ${secret_ref}" >&2
      exit 2
    fi

    redis-cli -h "${host}" -p "${port}" -a "${password}" PING >/dev/null
    redis-cli -h "${host}" -p "${port}" -a "${password}" CONFIG SET maxmemory "${max_memory}mb" >/dev/null
    redis-cli -h "${host}" -p "${port}" -a "${password}" CONFIG SET maxmemory-policy "${eviction_policy}" >/dev/null

    redis-cli -h "${host}" -p "${port}" -a "${password}" SET "${key_prefix}probe" "ok" >/dev/null
    redis-cli -h "${host}" -p "${port}" -a "${password}" GET "${key_prefix}probe" >/dev/null
    redis-cli -h "${host}" -p "${port}" -a "${password}" DEL "${key_prefix}probe" >/dev/null
    ...
    jq -n \
      ...
      --arg max_memory_mb "${max_memory}" \
      '{consumer:$consumer,provider:$provider,variant:$variant,status:$status,key_prefix:$key_prefix,max_memory_mb:$max_memory_mb}' >>"${items_file}"
  done
...
EOF_REPORT
  ...
  echo "PASS substrate apply (dragonfly): ${out_dir}"
}
