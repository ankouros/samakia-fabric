...
backup_file="${backup_dir}/definitions.json"
...
(out / "verification.json").write_text(json.dumps({"status": "verified", "definitions_keys": sorted(data.keys())}, indent=2, sort_keys=True) + "\n")
PY

echo "PASS rabbitmq restore verification (${restore_mode})"
