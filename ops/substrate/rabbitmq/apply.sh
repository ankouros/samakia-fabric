...
run_for_tenant() {
  ...
  contracts_json="$(list_enabled_contracts "${tenant_dir}" "rabbitmq")"
  echo "${contracts_json}" | jq -c '.[]' | while read -r entry; do
    ...
    if [[ "${resources}" == "{}" ]]; then
      echo "ERROR: resources are empty for ${tenant_id}/${consumer}/${provider}; specify vhost/user" >&2
      exit 2
    fi

    local secret_json admin_user admin_pass app_user app_pass vhost
...
PY
    ...
    vhost=$(python3 - <<PY
import json
print(json.loads('''${resources}''').get('vhost', '') or '')
PY
)

    if [[ -z "${vhost}" ]]; then
      echo "ERROR: resources.vhost is required for ${tenant_id}/${consumer}/${provider}" >&2
      exit 2
    fi
    ...
    curl -sf -u "${admin_user}:${admin_pass}" -H "content-type: application/json" \
      -X PUT "${base_url}/api/vhosts/${vhost}" -d '{}' >/dev/null

    curl -sf -u "${admin_user}:${admin_pass}" -H "content-type: application/json" \
      -X PUT "${base_url}/api/users/${app_user}" -d '{"password":"'"${app_pass}"'","tags":""}' >/dev/null

    curl -sf -u "${admin_user}:${admin_pass}" -H "content-type: application/json" \
      -X PUT "${base_url}/api/permissions/${vhost}/${app_user}" -d '{"configure":".*","write":".*","read":".*"}' >/dev/null

    curl -sf -u "${admin_user}:${admin_pass}" "${base_url}/api/healthchecks/node" >/dev/null
    ...
    jq -n \
      ...
      --arg vhost "${vhost}" \
      --arg user "${app_user}" \
      '{consumer:$consumer,provider:$provider,variant:$variant,status:$status,vhost:$vhost,user:$user}' >>"${items_file}"
  done
...
EOF_REPORT
  ...
  echo "PASS substrate apply (rabbitmq): ${out_dir}"
}
