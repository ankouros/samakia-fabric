# VM Golden Images

This section documents VM golden image contracts and validation.

Operator commands live in `../operator/cookbook.md`.

- `vm-golden-images.md`: contract model and lifecycle
- `image-lifecycle.md`: build → validate → reference
- `image-security.md`: security posture and acceptance checks
- `template-upgrades.md`: canonical upgrade semantics (replace/blue-green)
- `local-build-and-validate.md`: operator runbook for local build + validate
- `proxmox-template-registration.md`: guarded Proxmox template registration (Phase 8 Part 2)
